...
class NPE(NE):
    """Neural posterior estimation. Adapted from `sbi_jax` https://github.com/dirmeier/sbijax/.
    ...
    We wish to minimise the expected posterior entropy of an estimator 
        .. math::

        L = \mathbb{E}_{\theta \sim p(\theta, x)} \sum_i^{\rm batch} \log q(\theta | x)

    where:
        - :math:`x` is the input data
        - :math:`\theta` are the parameters of interest
